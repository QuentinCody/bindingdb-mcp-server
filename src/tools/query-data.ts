...
interface QueryEnv {
    BINDINGDB_DATA_DO?: unknown;
}

export function registerQueryData(server: McpServer, env?: QueryEnv): void {
    const handler = createQueryDataHandler("BINDINGDB_DATA_DO", "bindingdb");

    server.registerTool(
        "bindingdb_query_data",
        {
            title: "Query Staged BindingDB Data",
            description:
                "Query staged BindingDB data using SQL. Use this when responses are too large and have been staged with a data_access_id.",
            inputSchema: {
